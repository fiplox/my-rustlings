...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple > (0, 0, 0) && tuple < (255, 255, 255) {
            let c = (tuple.0 as u8, tuple.1 as u8, tuple.2 as u8);
            Ok(Color {
                red: c.0,
                green: c.1,
                blue: c.2,
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        if (arr[0], arr[1], arr[2]) > (0, 0, 0) && (arr[0], arr[1], arr[2]) < (255, 255, 255) {
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        if slice.len() != 3 {
            ...
            if (slice[0], slice[1], slice[2]) > (0, 0, 0)
                && (slice[0], slice[1], slice[2]) < (255, 255, 255)
            {
                let c = &[slice[0] as u8, slice[1] as u8, slice[2] as u8];
                Ok(Color {
                    red: c[0],
                    green: c[1],
                    blue: c[2],
